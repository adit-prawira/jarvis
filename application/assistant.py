from domain.senses.ear import Ear

NO_COMMAND_TIMEOUT_SECONDS = 30.0
POST_PROMPT_GRACE_SECONDS = 5.0
WELCOME_MESSAGE = "Welcome back, Sir"
ARE_YOU_THERE_MESSAGE = "Are you there, Sir?"

class Assistant:
    """Application service: orchestrate JARVIS senses and the brain"""

    def __init__(self, ear: Ear) -> None:
        self._ear = ear
        self._is_welcomed = False

    def run(self) -> None:
        print("Listening for 'hey jarvis'...")
        while True:
            self._listen_for_a_command()

    def _listen_for_a_command(self) -> None:
        self._ear.listen_for_wake_command()
        if not self._is_welcomed:
            print(WELCOME_MESSAGE)
            self._is_welcomed = True
        utterance = self._ear.transcribe_utterance(timeout=NO_COMMAND_TIMEOUT_SECONDS)
        if utterance is None:
            print(ARE_YOU_THERE_MESSAGE)
            utterance = self._ear.transcribe_utterance(timeout=POST_PROMPT_GRACE_SECONDS)
            if utterance is None:
                return
        print(f"Transcribed: {utterance}")
